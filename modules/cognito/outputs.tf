...
output "user_pool_domain" {
  ...
  value       = "https://${aws_cognito_user_pool_domain.default.domain}.auth.${data.aws_region.current.name}.amazoncognito.com"
}

output "user_pool_endpoint" {
  description = "JWKS endpoint URL for JWT validation (e.g. in Flask)"
  value       = "https://cognito-idp.${data.aws_region.current.name}.amazonaws.com/${aws_cognito_user_pool.default.id}"
}
